				...
				$("#mreg").click(function(){
					if(mreg_i!=0){
						if(regnamenum==0 || regnamenum1==0){
							...
							$("#regpsd3").removeClass("regcolor");
							$("#newregpsd1").removeClass("regcolor");
							$("#regname").attr("placeholder","Phone number");
							regmove("#regimg");
							var regname=$("#regname").val();
							$("#loginname").val(regname);
							setTimeout(function(){
								loginclick("#login");
								$("#regname").val("");
								$("#regpsd").val("");
...
//								黑板留言介绍
								...
								ctx1.beginPath();
								ctx1.fillStyle="#2c3a36";
								ctx1.fill();
								ctx1.fillRect(0,0,519,308);
								ctx1.closePath();
								ctx1.font="20px Arial";
								ctx1.fillStyle="#dde295";
								ctx1.fillText("每次登陆只有两次留言机会哦！",20,40);
								ctx1.fillText("双击清空黑板开始留言!",20,80);
							}else{
								$("#loginpsd").val("");
								$("#loginpsd").attr("placeholder","密码输入错误，请重新输入！");
							}
						});
					...
					$.post("servlet/validatephoneservlet",
							{
								...
								if(data==1){
									$("#regname").val("");
									$("#regname").attr("placeholder","该手机号已注册");
									regnamenum=0;
									regnamenum1=0;
									$("#regphone1").removeClass("regcolor");
									$("#regphone2").removeClass("regcolor");
					...
					$.post("servlet/validatephoneservlet",
							{
								...
								if(data==0){
									$("#loginname").val("");
									$("#loginname").attr("placeholder","该用户不存在，请重新输入！");
								}
							});
				}
					...
					$.post("servlet/addmsgconservlet",
							{
								...
								if(data==1){
									...
									if(msgcount==1){
										...
										ctx1.font="20px Arial";
										ctx1.fillStyle="#dde295";
										ctx1.fillText("留言成功！本次登录还有一次留言机会！",20,40);
										ctx1.fillText("双击清空黑板继续留言!",20,80);
									}
									if(msgcount==2){
